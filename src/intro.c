...
static void CB2_CustomOpeningIntro(void);
static void CB2_CustomOpeningIntroExit(void);
static void Task_CustomOpeningIntro(u8);
static void SetUpCustomOpeningIntroScreen(void);
static void PrintCenteredCustomIntroLine(const u8 *str, u8 y);
...
enum {
    ...
    COPYRIGHT_START_DISCLAIMER,
    COPYRIGHT_DISCLAIMER_SHOW,
    COPYRIGHT_DISCLAIMER_END,
    COPYRIGHT_START_INTRO,
};

#define DISCLAIMER_DURATION 720

#define CUSTOM_INTRO_TOTAL_DURATION 600
#define CUSTOM_INTRO_SKIP_DELAY 120
#define CUSTOM_INTRO_SCENE_1_END 150
#define CUSTOM_INTRO_SCENE_2_END 300
#define CUSTOM_INTRO_SCENE_3_END 450

static const struct BgTemplate sDisclaimerBgTemplates[] =
{
    {
        .bg = 0,
        .charBaseIndex = 0,
...
static const struct WindowTemplate sDisclaimerWindowTemplates[] =
{
    {
        .bg = 0,
        .tilemapLeft = 0,
...
static const u8 sDisclaimerTextColor[] = {TEXT_COLOR_TRANSPARENT, TEXT_COLOR_LIGHT_GRAY, TEXT_COLOR_DARK_GRAY};
static const u8 sText_DisclaimerLine1[] = _("Esta rom fue hecha");
static const u8 sText_DisclaimerLine2[] = _("gracias a");
static const u8 sText_DisclaimerLine3[] = _("MalakLegends.");
static const u8 sText_DisclaimerLine4[] = _("Proyecto fan");
static const u8 sText_DisclaimerLine5[] = _("sin fines de lucro.");
static const u8 sText_DisclaimerLine6[] = _("Pokemon es propiedad de");
static const u8 sText_DisclaimerLine7[] = _("Nintendo, Game Freak y");
static const u8 sText_DisclaimerLine8[] = _("The Pokemon Company.");
static const u8 sText_DisclaimerLine9[] = _("Gracias por ser");
static const u8 sText_DisclaimerLine10[] = _("parte de ella.");

static const u8 sText_CustomIntro_YearsLater[] = _("anos despues...");
static const u8 sText_CustomIntro_LandSight[] = _("tierra a la vista");
static const u8 sText_CustomIntro_Boat[] = _("barco en camino");
static const u8 sText_CustomIntro_Land[] = _("costa");
static const u8 sText_CustomIntro_Chars1[] = _("surgen personajes destacados");
static const u8 sText_CustomIntro_Chars2[] = _("de kanto, johto y hoenn");
static const u8 sText_CustomIntro_Epics1[] = _("despiertan los pokemon epicos");
static const u8 sText_CustomIntro_Epics2[] = _("el destino vuelve a llamarte");
static const u8 sText_CustomIntro_Run1[] = _("el protagonista esta listo");
static const u8 sText_CustomIntro_Run2A[] = _("corriendo...");
static const u8 sText_CustomIntro_Run2B[] = _("corriendo");

static EWRAM_DATA u8 sCustomIntroBrendanSpriteId = 0;
static EWRAM_DATA u8 sCustomIntroMaySpriteId = 0;
static EWRAM_DATA u8 sCustomIntroFlygonSpriteId = 0;
static EWRAM_DATA u8 sCustomIntroHeroSpriteId = 0;

static void SetUpDisclaimerScreen(void)
{
    u8 x;
    ...
    InitBgsFromTemplates(0, sDisclaimerBgTemplates, ARRAY_COUNT(sDisclaimerBgTemplates));
    InitWindows(sDisclaimerWindowTemplates);
    DeactivateAllTextPrinters();
    FillWindowPixelBuffer(0, PIXEL_FILL(0));
    LoadPalette(gStandardMenuPalette, BG_PLTT_ID(15), PLTT_SIZE_4BPP);

    x = GetStringCenterAlignXOffset(FONT_NORMAL, sText_DisclaimerLine1, DISPLAY_WIDTH);
    AddTextPrinterParameterized4(0, FONT_NORMAL, x, 4, 0, 0, sDisclaimerTextColor, TEXT_SKIP_DRAW, sText_DisclaimerLine1);
    x = GetStringCenterAlignXOffset(FONT_NORMAL, sText_DisclaimerLine2, DISPLAY_WIDTH);
    AddTextPrinterParameterized4(0, FONT_NORMAL, x, 17, 0, 0, sDisclaimerTextColor, TEXT_SKIP_DRAW, sText_DisclaimerLine2);
    x = GetStringCenterAlignXOffset(FONT_NORMAL, sText_DisclaimerLine3, DISPLAY_WIDTH);
    AddTextPrinterParameterized4(0, FONT_NORMAL, x, 30, 0, 0, sDisclaimerTextColor, TEXT_SKIP_DRAW, sText_DisclaimerLine3);
    x = GetStringCenterAlignXOffset(FONT_NORMAL, sText_DisclaimerLine4, DISPLAY_WIDTH);
    AddTextPrinterParameterized4(0, FONT_NORMAL, x, 49, 0, 0, sDisclaimerTextColor, TEXT_SKIP_DRAW, sText_DisclaimerLine4);
    x = GetStringCenterAlignXOffset(FONT_NORMAL, sText_DisclaimerLine5, DISPLAY_WIDTH);
    AddTextPrinterParameterized4(0, FONT_NORMAL, x, 62, 0, 0, sDisclaimerTextColor, TEXT_SKIP_DRAW, sText_DisclaimerLine5);
    x = GetStringCenterAlignXOffset(FONT_NORMAL, sText_DisclaimerLine6, DISPLAY_WIDTH);
    AddTextPrinterParameterized4(0, FONT_NORMAL, x, 83, 0, 0, sDisclaimerTextColor, TEXT_SKIP_DRAW, sText_DisclaimerLine6);
    x = GetStringCenterAlignXOffset(FONT_NORMAL, sText_DisclaimerLine7, DISPLAY_WIDTH);
    AddTextPrinterParameterized4(0, FONT_NORMAL, x, 96, 0, 0, sDisclaimerTextColor, TEXT_SKIP_DRAW, sText_DisclaimerLine7);
    x = GetStringCenterAlignXOffset(FONT_NORMAL, sText_DisclaimerLine8, DISPLAY_WIDTH);
    AddTextPrinterParameterized4(0, FONT_NORMAL, x, 109, 0, 0, sDisclaimerTextColor, TEXT_SKIP_DRAW, sText_DisclaimerLine8);
    x = GetStringCenterAlignXOffset(FONT_NORMAL, sText_DisclaimerLine9, DISPLAY_WIDTH);
    AddTextPrinterParameterized4(0, FONT_NORMAL, x, 132, 0, 0, sDisclaimerTextColor, TEXT_SKIP_DRAW, sText_DisclaimerLine9);
    x = GetStringCenterAlignXOffset(FONT_NORMAL, sText_DisclaimerLine10, DISPLAY_WIDTH);
    AddTextPrinterParameterized4(0, FONT_NORMAL, x, 145, 0, 0, sDisclaimerTextColor, TEXT_SKIP_DRAW, sText_DisclaimerLine10);
...
// Task data for Task_CustomOpeningIntro
#define tCustomTimer data[0]
#define tBoatX data[1]

static void PrintCenteredCustomIntroLine(const u8 *str, u8 y)
{
    u8 x = GetStringCenterAlignXOffset(FONT_NORMAL, str, DISPLAY_WIDTH);
    AddTextPrinterParameterized4(0, FONT_NORMAL, x, y, 0, 0, sDisclaimerTextColor, TEXT_SKIP_DRAW, str);
}
...
static void SetUpCustomOpeningIntroScreen(void)
{
    ...
    sCustomIntroBrendanSpriteId = 0xFF;
    sCustomIntroMaySpriteId = 0xFF;
    sCustomIntroFlygonSpriteId = 0xFF;
    sCustomIntroHeroSpriteId = 0xFF;
    ...
    sCustomIntroBrendanSpriteId = CreateIntroBrendanSprite(72, 98);
    sCustomIntroMaySpriteId = CreateIntroMaySprite(168, 98);
    sCustomIntroFlygonSpriteId = CreateIntroFlygonSprite(120, 64);
    sCustomIntroHeroSpriteId = CreateIntroBrendanSprite(120, 96);

    for (spriteId = 0; spriteId < MAX_SPRITES; spriteId++)
    {
        if (spriteId == sCustomIntroBrendanSpriteId
         || spriteId == sCustomIntroMaySpriteId
         || spriteId == sCustomIntroFlygonSpriteId
         || spriteId == sCustomIntroHeroSpriteId)
...
static void Task_CustomOpeningIntro(u8 taskId)
{
    struct Task *task = &gTasks[taskId];
    const u8 *runAnimText;

    task->tCustomTimer++;

    if (task->tCustomTimer >= CUSTOM_INTRO_SKIP_DELAY
     && JOY_NEW(A_BUTTON | B_BUTTON | START_BUTTON | SELECT_BUTTON))
    {
        BeginNormalPaletteFade(PALETTES_ALL, 0, 0, 16, RGB_BLACK);
        DestroyTask(taskId);
        SetMainCallback2(CB2_CustomOpeningIntroExit);
        return;
    }

    FillWindowPixelBuffer(0, PIXEL_FILL(0));

    if (task->tCustomTimer < CUSTOM_INTRO_SCENE_1_END)
    {
        if (sCustomIntroBrendanSpriteId < MAX_SPRITES)
            gSprites[sCustomIntroBrendanSpriteId].invisible = TRUE;
        if (sCustomIntroMaySpriteId < MAX_SPRITES)
            gSprites[sCustomIntroMaySpriteId].invisible = TRUE;
        if (sCustomIntroFlygonSpriteId < MAX_SPRITES)
            gSprites[sCustomIntroFlygonSpriteId].invisible = TRUE;
        if (sCustomIntroHeroSpriteId < MAX_SPRITES)
            gSprites[sCustomIntroHeroSpriteId].invisible = TRUE;

        if ((task->tCustomTimer & 3) == 0 && task->tBoatX < 132)
            task->tBoatX += 2;

        PrintCenteredCustomIntroLine(sText_CustomIntro_YearsLater, 16);
        PrintCenteredCustomIntroLine(sText_CustomIntro_LandSight, 34);
        AddTextPrinterParameterized4(0, FONT_NORMAL, task->tBoatX, 74, 0, 0, sDisclaimerTextColor, TEXT_SKIP_DRAW, sText_CustomIntro_Boat);
        AddTextPrinterParameterized4(0, FONT_NORMAL, 144, 60, 0, 0, sDisclaimerTextColor, TEXT_SKIP_DRAW, sText_CustomIntro_Land);
    }
    else if (task->tCustomTimer < CUSTOM_INTRO_SCENE_2_END)
    {
        if (sCustomIntroBrendanSpriteId < MAX_SPRITES)
            gSprites[sCustomIntroBrendanSpriteId].invisible = FALSE;
        if (sCustomIntroMaySpriteId < MAX_SPRITES)
            gSprites[sCustomIntroMaySpriteId].invisible = FALSE;
        if (sCustomIntroFlygonSpriteId < MAX_SPRITES)
            gSprites[sCustomIntroFlygonSpriteId].invisible = TRUE;
        if (sCustomIntroHeroSpriteId < MAX_SPRITES)
            gSprites[sCustomIntroHeroSpriteId].invisible = TRUE;

        PrintCenteredCustomIntroLine(sText_CustomIntro_Chars1, 44);
        PrintCenteredCustomIntroLine(sText_CustomIntro_Chars2, 62);
    }
    else if (task->tCustomTimer < CUSTOM_INTRO_SCENE_3_END)
    {
        if (sCustomIntroBrendanSpriteId < MAX_SPRITES)
            gSprites[sCustomIntroBrendanSpriteId].invisible = TRUE;
        if (sCustomIntroMaySpriteId < MAX_SPRITES)
            gSprites[sCustomIntroMaySpriteId].invisible = TRUE;
        if (sCustomIntroFlygonSpriteId < MAX_SPRITES)
            gSprites[sCustomIntroFlygonSpriteId].invisible = FALSE;
        if (sCustomIntroHeroSpriteId < MAX_SPRITES)
            gSprites[sCustomIntroHeroSpriteId].invisible = TRUE;

        PrintCenteredCustomIntroLine(sText_CustomIntro_Epics1, 44);
        PrintCenteredCustomIntroLine(sText_CustomIntro_Epics2, 62);
    }
    else
    {
        if (sCustomIntroBrendanSpriteId < MAX_SPRITES)
        ...
        if (sCustomIntroHeroSpriteId < MAX_SPRITES)
        {
            gSprites[sCustomIntroHeroSpriteId].invisible = FALSE;
            gSprites[sCustomIntroHeroSpriteId].x2 = (task->tCustomTimer & 4) ? 1 : -1;
            gSprites[sCustomIntroHeroSpriteId].y2 = (task->tCustomTimer & 8) ? 1 : 0;
        }

        runAnimText = (task->tCustomTimer & 8) ? sText_CustomIntro_Run2A : sText_CustomIntro_Run2B;
        PrintCenteredCustomIntroLine(sText_CustomIntro_Run1, 44);
        PrintCenteredCustomIntroLine(runAnimText, 62);
    }

    CopyWindowToVram(0, COPYWIN_FULL);

    if (task->tCustomTimer >= CUSTOM_INTRO_TOTAL_DURATION)
    {
        BeginNormalPaletteFade(PALETTES_ALL, 0, 0, 16, RGB_BLACK);
        DestroyTask(taskId);
        SetMainCallback2(CB2_CustomOpeningIntroExit);
...
static u8 SetUpCopyrightScreen(void)
{
    ...
    case COPYRIGHT_START_FADE:
        ...
        break;
    case COPYRIGHT_START_DISCLAIMER:
        if (UpdatePaletteFade())
            break;
        ...
        gMain.state++;
        break;
    case COPYRIGHT_DISCLAIMER_SHOW:
        if (UpdatePaletteFade())
            break;
        gIntroFrameCounter++;
        if (gIntroFrameCounter > DISCLAIMER_DURATION)
        {
            BeginNormalPaletteFade(PALETTES_ALL, 0, 0, 16, RGB_BLACK);
            gMain.state++;
        }
        break;
    case COPYRIGHT_DISCLAIMER_END:
        if (UpdatePaletteFade())
            break;
    ...
    case COPYRIGHT_START_INTRO:
        if (UpdatePaletteFade())
            break;
        SetUpCustomOpeningIntroScreen();
        CreateTask(Task_CustomOpeningIntro, 0);
        BeginNormalPaletteFade(PALETTES_ALL, 0, 16, 0, RGB_BLACK);
        SetMainCallback2(CB2_CustomOpeningIntro);
