...
static const u8 gText_TextSpeedFast[]      = _("{COLOR GREEN}{SHADOW LIGHT_GREEN}Rapido");
static const u8 gText_BattleSceneOn[]      = _("{COLOR GREEN}{SHADOW LIGHT_GREEN}Activa");
static const u8 gText_BattleSceneOff[]     = _("{COLOR GREEN}{SHADOW LIGHT_GREEN}Oculta");
...
{
    [MENUITEM_TEXTSPEED]   = COMPOUND_STRING("Velocidad texto"),
    [MENUITEM_BATTLESCENE] = COMPOUND_STRING("Escena batalla"),
    [MENUITEM_BATTLESTYLE] = COMPOUND_STRING("Estilo batalla"),
    [MENUITEM_SOUND]       = COMPOUND_STRING("Sonido"),
    [MENUITEM_BUTTONMODE]  = COMPOUND_STRING("Modo botones"),
...
static void DrawHeaderText(void)
{
    ...
    FillWindowPixelRect(WIN_HEADER, PIXEL_FILL(2), 4, 3, 10, 10);
    FillWindowPixelRect(WIN_HEADER, PIXEL_FILL(3), 20, 3, 164, 2);
    FillWindowPixelRect(WIN_HEADER, PIXEL_FILL(3), 20, 11, 164, 2);
    titleX = GetStringCenterAlignXOffset(FONT_NORMAL, gText_Option, 208);
    AddTextPrinterParameterized(WIN_HEADER, FONT_NORMAL, gText_Option, titleX, 1, TEXT_SKIP_DRAW, NULL);
...
static void DrawOptionMenuTexts(void)
{
    ...
    FillWindowPixelRect(WIN_OPTIONS, PIXEL_FILL(2), 4, 4, 10, 104);

    for (i = 0; i < MENUITEM_COUNT; i++)
    {
        u8 y = i * 16;

        FillWindowPixelRect(WIN_OPTIONS, PIXEL_FILL(3), 20, y + 3, 72, 10);
        if (i != MENUITEM_CANCEL)
            FillWindowPixelRect(WIN_OPTIONS, PIXEL_FILL(2), 98, y + 3, 100, 10);
        if (i != MENUITEM_CANCEL)
            FillWindowPixelRect(WIN_OPTIONS, PIXEL_FILL(2), 6, y + 14, 194, 1);
    }
