...
{
    static const u8 sSpriteToFacilityClass[CHAR_SPRITE_COUNT] = {
        FACILITY_CLASS_BRENDAN,
        FACILITY_CLASS_MAY,
        FACILITY_CLASS_RED,
        FACILITY_CLASS_LEAF,
    };
    u8 idx = gSaveBlock2Ptr->playerSpriteIndex;
    if (idx >= CHAR_SPRITE_COUNT)
        idx = (playerGender == MALE) ? CHAR_SPRITE_BRENDAN : CHAR_SPRITE_MAY;
    return FacilityClassToPicIndex(sSpriteToFacilityClass[idx]);
}
