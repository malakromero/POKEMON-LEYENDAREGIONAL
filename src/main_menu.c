...
static EWRAM_DATA u16 sMainMenuBgScrollX = 0;
static EWRAM_DATA u8 sMainMenuBoatSpriteId = 0;
...
static void DrawMainMenuOptionWindow(u8 windowId, const u8 *title, bool8 featured, bool8 selected);
static void DrawMainMenuContinueWindow(bool8 selected);
static void RedrawMainMenuWindows(u8 taskId);
static void CreateMainMenuDecorSprites(void);
static void SpriteCB_MainMenuBoat(struct Sprite *sprite);
...
static const u16 sMainMenuSeaTiles[] = INCBIN_U16("graphics/seagallop/water.4bpp");
static const u16 sMainMenuBoatTiles[] = INCBIN_U16("graphics/seagallop/ferry_sprite.4bpp");
static const u16 sMainMenuSeaTilemap[] = INCBIN_U16("graphics/seagallop/wb_tilemap.bin");
static const u16 sMainMenuSeaPal[] = INCBIN_U16("graphics/seagallop/water.gbapal");
static const u16 sMainMenuBoatSilhouettePal[] =
{
    RGB_BLACK, RGB(4, 4, 6), RGB(6, 7, 9), RGB(8, 9, 11),
    RGB(10, 11, 13), RGB(12, 13, 15), RGB(14, 15, 17), RGB(16, 17, 19),
    RGB(18, 19, 21), RGB(20, 21, 23), RGB(22, 23, 25), RGB(24, 25, 27),
    RGB(26, 27, 29), RGB(28, 29, 30), RGB(30, 30, 31), RGB_WHITE
};

static const u8 gText_SaveFileCorrupted[] = _("El archivo guardado esta dañado.\nSe cargara la partida anterior.");
static const u8 gText_SaveFileErased[] = _("El archivo guardado fue borrado\npor corrupcion o daño.");
static const u8 gJPText_No1MSubCircuit[] = _("1Mサブきばんが ささっていません！");
static const u8 gText_BatteryRunDry[] = _("La bateria interna se agoto.\nEl juego puede continuar.\pSin embargo, los eventos por reloj\nya no ocurriran.");
...
static const u8 gText_MainMenuMysteryGift[] = _("Regalo misterio");
static const u8 gText_MainMenuMysteryGift2[] = _("Regalo misterio");
static const u8 gText_MainMenuMysteryEvents[] = _("Eventos misterio");
static const u8 gText_WirelessNotConnected[] = _("El Adaptador Inalambrico no\nesta conectado.");
static const u8 gText_MysteryGiftCantUse[] = _("No puedes usar REGALO MISTERIO\ncon el Adaptador Inalambrico puesto.");
static const u8 gText_MysteryEventsCantUse[] = _("No puedes usar EVENTOS MISTERIO\ncon el Adaptador Inalambrico puesto.");
...
static const u8 gText_MainMenuNewGameSub[] = _("Comienza tu aventura");
static const u8 gText_MainMenuContinueSub[] = _("Retoma tu viaje actual");
static const u8 gText_MainMenuOptionSub[] = _("Ajusta la experiencia");
static const u8 gText_MainMenuMysteryGiftSub[] = _("Contenido especial");
static const u8 gText_MainMenuMysteryEventsSub[] = _("Eventos externos");
...
enum
{
    TAG_MAIN_MENU_BOAT = 3000,
};

#define MAIN_MENU_BORDER_TILE   0x1D5
#define BIRCH_DLG_BASE_TILE_NUM 0xFC

static const struct OamData sOamData_MainMenuBoat =
{
...
static const union AnimCmd sAnim_MainMenuBoat[] =
{
    ANIMCMD_FRAME(0, 30),
    ANIMCMD_END,
};

static const union AnimCmd *const sAnimTable_MainMenuBoat[] =
{
    sAnim_MainMenuBoat,
};

static const struct SpriteTemplate sSpriteTemplate_MainMenuBoat =
{
    .tileTag = TAG_MAIN_MENU_BOAT,
    .paletteTag = TAG_MAIN_MENU_BOAT,
    .oam = &sOamData_MainMenuBoat,
    .anims = sAnimTable_MainMenuBoat,
    .callback = SpriteCB_MainMenuBoat,
};

static const struct SpriteSheet sSpriteSheet_MainMenuBoat[] =
{
    {
        .data = sMainMenuBoatTiles,
        .size = sizeof(sMainMenuBoatTiles),
        .tag = TAG_MAIN_MENU_BOAT,
    },
    {},
};

static const struct SpritePalette sSpritePalette_MainMenuBoat[] =
{
    {
        .data = sMainMenuBoatSilhouettePal,
        .tag = TAG_MAIN_MENU_BOAT,
    },
    {},
};
...
static void CB2_MainMenu(void)
{
    ...
    BuildOamBuffer();
    UpdatePaletteFade();
    sMainMenuBgScrollX += 0x40;
    ChangeBgX(1, sMainMenuBgScrollX, BG_COORD_SET);
}

static void CB2_NewGameBirchRunner(void)
{
    RunTasks();
    AnimateSprites();
    BuildOamBuffer();
...
static void VBlankCB_NewGameBirchRunner(void)
{
    LoadOam();
...
void CB2_StartNewGameBirchSpeech(void)
{
    SetVBlankCallback(NULL);
    ResetTasks();
    ResetSpriteData();
    FreeAllSpritePalettes();
    SetVBlankCallback(VBlankCB_NewGameBirchRunner);
    SetMainCallback2(CB2_NewGameBirchRunner);
...
static u32 InitMainMenu(bool8 returningFromOptionsMenu)
{
    ...
    ResetSpriteData();
    FreeAllSpritePalettes();
    sMainMenuBoatSpriteId = SPRITE_NONE;
    ...
    LoadBgTiles(1, sMainMenuSeaTiles, sizeof(sMainMenuSeaTiles), 0);
    CopyToBgTilemapBufferRect(1, sMainMenuSeaTilemap, 0, 0, 32, 32);
    LoadPalette(sMainMenuSeaPal, BG_PLTT_ID(4), sizeof(sMainMenuSeaPal));
    CopyBgTilemapBufferToVram(1);
    InitWindows(sWindowTemplates_MainMenu);
    DeactivateAllTextPrinters();
    LoadMainMenuWindowFrameTiles(0, MAIN_MENU_BORDER_TILE);
    sMainMenuBgScrollX = 0;
    LoadSpriteSheets(sSpriteSheet_MainMenuBoat);
    LoadSpritePalettes(sSpritePalette_MainMenuBoat);
    ...
    SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_MODE_0 | DISPCNT_BG0_ON | DISPCNT_BG1_ON | DISPCNT_WIN0_ON | DISPCNT_OBJ_ON | DISPCNT_OBJ_1D_MAP);
    ShowBg(0);
    ShowBg(1);
...
static void Task_NewGameBirchSpeech_WaitForPlayerFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        if (gSkipBirchPlayerSetup)
        {
            u8 spriteId;

            gSkipBirchPlayerSetup = FALSE;
            if (gSaveBlock2Ptr->playerGender != MALE)
...
static void DrawMainMenuOptionWindow(u8 windowId, const u8 *title, bool8 featured, bool8 selected)
{
    ...
    if (featured)
    {
        FillWindowPixelRect(windowId, PIXEL_FILL(0xB), 4, 4, width - 8, height - 8);
        FillWindowPixelRect(windowId, PIXEL_FILL(0xA), 4, 4, 10, height - 8);
        if (selected)
        {
            FillWindowPixelRect(windowId, PIXEL_FILL(0xC), 18, 4, width - 26, 3);
            FillWindowPixelRect(windowId, PIXEL_FILL(0xC), 18, height - 7, width - 26, 3);
        }
    }
    else
    {
    ...
    titleX = GetStringCenterAlignXOffset(FONT_NORMAL, title, width - 8) + 4;
    AddTextPrinterParameterized3(windowId, FONT_NORMAL, titleX, titleY, sTextColor_Headers, TEXT_SKIP_DRAW, title);
}

static void DrawMainMenuContinueWindow(bool8 selected)
{
    u16 width = GetWindowAttribute(2, WINDOW_WIDTH) * 8;
    u8 titleX;
    u8 subtitleX;

    FillWindowPixelBuffer(2, PIXEL_FILL(1));
    FillWindowPixelRect(2, PIXEL_FILL(0xA), 4, 4, width - 8, 40);
    FillWindowPixelRect(2, PIXEL_FILL(0xC), 4, 4, 10, 40);
    if (selected)
    {
        FillWindowPixelRect(2, PIXEL_FILL(0xB), 18, 4, width - 26, 3);
        FillWindowPixelRect(2, PIXEL_FILL(0xB), 18, 41, width - 26, 3);
    }

    titleX = GetStringCenterAlignXOffset(FONT_NORMAL, gText_MainMenuContinue, width - 8) + 4;
    subtitleX = GetStringCenterAlignXOffset(FONT_SMALL, gText_MainMenuContinueSub, width - 8) + 4;
    AddTextPrinterParameterized3(2, FONT_NORMAL, titleX, 2, sTextColor_Headers, TEXT_SKIP_DRAW, gText_MainMenuContinue);
    AddTextPrinterParameterized3(2, FONT_SMALL, subtitleX, 18, sTextColor_MenuInfo, TEXT_SKIP_DRAW, gText_MainMenuContinueSub);
}
...
static void RedrawMainMenuWindows(u8 taskId)
{
    ...
    case HAS_NO_SAVED_GAME:
    default:
        DrawMainMenuOptionWindow(0, gText_MainMenuNewGame, TRUE, data[1] == 0);
        DrawMainMenuOptionWindow(1, gText_MainMenuOption, FALSE, data[1] == 1);
        PutWindowTilemap(0);
        PutWindowTilemap(1);
        CopyWindowToVram(0, COPYWIN_GFX);
        ...
        break;
    case HAS_SAVED_GAME:
        DrawMainMenuContinueWindow(data[1] == 0);
        DrawMainMenuOptionWindow(3, gText_MainMenuNewGame, TRUE, data[1] == 1);
        DrawMainMenuOptionWindow(4, gText_MainMenuOption, FALSE, data[1] == 2);
    ...
    case HAS_MYSTERY_GIFT:
        DrawMainMenuContinueWindow(data[1] == 0);
        DrawMainMenuOptionWindow(3, gText_MainMenuNewGame, TRUE, data[1] == 1);
        DrawMainMenuOptionWindow(4, gText_MainMenuMysteryGift, FALSE, data[1] == 2);
        DrawMainMenuOptionWindow(5, gText_MainMenuOption, FALSE, data[1] == 3);
        MainMenu_FormatSavegameText();
        PutWindowTilemap(2);
        PutWindowTilemap(3);
        PutWindowTilemap(4);
    ...
    case HAS_MYSTERY_EVENTS:
        DrawMainMenuContinueWindow(data[1] == 0);
        DrawMainMenuOptionWindow(3, gText_MainMenuNewGame, TRUE, data[1] == 1);
        DrawMainMenuOptionWindow(4, gText_MainMenuMysteryGift2, FALSE, data[1] == 2);
        DrawMainMenuOptionWindow(5, gText_MainMenuMysteryEvents, FALSE, data[1] == 3);
        DrawMainMenuOptionWindow(6, gText_MainMenuOption, FALSE, data[1] == 4);
        MainMenu_FormatSavegameText();
        PutWindowTilemap(2);
        PutWindowTilemap(3);
        PutWindowTilemap(4);
...
static void CreateMainMenuDecorSprites(void)
{
    if (sMainMenuBoatSpriteId == SPRITE_NONE)
    {
        sMainMenuBoatSpriteId = CreateSprite(&sSpriteTemplate_MainMenuBoat, 196, 118, 2);
        gSprites[sMainMenuBoatSpriteId].oam.priority = 2;
        gSprites[sMainMenuBoatSpriteId].data[0] = 0;
        gSprites[sMainMenuBoatSpriteId].data[1] = 64;
    }
}

static void SpriteCB_MainMenuBoat(struct Sprite *sprite)
{
    sprite->x2 = Sin(sprite->data[0], 5);
    sprite->y2 = Sin(sprite->data[1], 2);
    sprite->data[0] = (sprite->data[0] + 1) & 0xFF;
    sprite->data[1] = (sprite->data[1] + 3) & 0xFF;
}
